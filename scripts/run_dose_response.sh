#!/usr/bin/env bash
# L3a dose-response. Spark comes from /opt/spark; the venv supplies only
# the Python libraries, so there is one unambiguous Spark version in play.
set -euo pipefail
cd "$(dirname "$0")/.."
...
exec "$SPARK_HOME/bin/spark-submit" \
  ...
  src/raincheck/jobs/run_dose_response.py "$@"
